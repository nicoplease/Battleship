...
class Player
  attr_reader :ship_spots, :valid_guesses, :placed
  def initialize
    @ship_spots = nil
    @placed = []
    @empty_spot = nil
  ...
  def place_user_destroyer(place) # make sure string passed into it is upcase
  ...
  def place_user_submarine(place) # make sure string passed into it is upcase
...
# player = Player.new
# player.place_user_destroyer('A1 D1')
# p player.placed
